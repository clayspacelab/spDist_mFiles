...
% %             
%             if dtheta_poldeg(ss,cc,nr) > 27
%                 dtheta_poldeg(ss,cc,nr) = nan;
%                 mu(:,ss,cc,nr) = nan; 
%             else
%             end 
...
for ss = 6
    ...
    for cc = 2 %1:length(cond)
        ...
        for vv = 1:length(ROIs)
            ...
            for dd = 3 %1:length(delay_tpts)
                ...
                text(max(xlim)-(.8*max(xlim)),max(ylim)-(1.9*max(ylim)),sprintf('r = %.2f',rho_theta(cc,ss,vv,dd)),'FontSize',14)
            end
 ...
 set(get(gcf,'Children'),'XLim',[-180 180],'YLim',[-30 30],'xtick',-180:90:180) %,'xtick',{'180','0','180'},
...
%                 
%                 [p,~,statsth] = signrank(fish_zthperm(cc,:,vv,dd,xx));
%                 shuff_Tth(cc,vv,dd,xx) = statsth.signedrank;
                
%                 
%                      [p,~,statsth] = signrank(rho_permth(cc,:,vv,dd,xx));
%                      shuff_Tth(cc,vv,dd,xx) = statsth.signedrank;
            end
        end
    end
