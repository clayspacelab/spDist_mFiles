...
for ff = 1:length(cond_to_plot)
    for vv = 1:length(ROIs)
        ...
        for ss = 1:length(subj)
            ...
            %thatidx = all_subj==ss & all_ROIs==vv & all_conds(:,1)==2 &
            %all_conds(:,6)==2 & this_rel > 0; %        figure % this is geh checking things, on 10/21/20
            %thisd2(:,:,ss) = squeeze(mean(all_recons_flipped(thatidx,:,:),1)).';
...
%        figure % this is geh checking things, on 10/21/20
%        for sbj = 1:7
%          hold on; plot(linspace(-180,180,90),mean(mean(thisd(tpts_to_plot,:,sbj),3),1),'LineWidth',2,'color',cond_colors(ff,:))
%         hold on; plot(linspace(-180,180,90),mean(mean(thisd2(tpts_to_plot,:,sbj),3),1),'LineWidth',2,'color','r') % pos
%        end 
