    ...
    for vv = 1:length(ROIs)
        ...
        for cc = 1:length(cu)
           ...
           %btwn_fill = [(mean(thisdata,1))+1.*my_sem fliplr((mean(thisdata,1))-1.*fliplr(my_sem))]; % TCS: fliplr also for my_sem?
            btwn_fill = [(mean(thisdata,1))+1.*my_sem fliplr((mean(thisdata,1)-1.*(my_sem)))]; % geh test
...
for vv = 1:length(ROIs)
    ...
    for dd =1:length(delay_tpts)
        ...
        if plot_indiv == 1
            ...
            hold on;
            plot(dd+hoffset*-1,squeeze(thisd(dd,1,:)),'.','markersize',10,'Color',dist_colors(1,:));
            hold on;
            plot(dd+hoffset*1,squeeze(thisd(dd,2,:)),'.','markersize',10,'Color',dist_colors(2,:));
        end
    ...
    for cc =1:length(cu)
        ...
        h(cc) = plot([1 2 3],mean(thisd(:,cc,:),3)','o-','color',cond_colors(cc,:),'linewidth',2,'MarkerSize',8,'MarkerFaceColor',cond_colors(cc,:));
 
        for dd =1:length(delay_tpts)
            plot([dd dd],[mean(thisd(dd,cc,:),3)+1.*my_sem(dd,cc) mean(thisd(dd,cc,:),3)-1.*my_sem(dd,cc)], '-','LineWidth',2,'color',cond_colors(cc,:))
        end
    end
    clear thisidx
    clear thisd
   
    line([0 4], [0 0], 'color',[0 0 0],'linewidth',0.5,'linestyle','-') % geh add y =0 baseline 
...
thissubj=subj_var_3;
thisroi =roi_var_3;
[P_truth_3,T_truth_3,~] = anovan(thisy,{thissubj,thisepoch,thiscond,thisroi},'model','full','random',1,'varnames',{'subj','epoch','cond','roi'});
...
sig_mrkr ={'o','+','x'}; %epoch, cond, epoch x cond int
y_mod = [.1 .15 .2];
for vv = 1:length(ROIs)
    for ee =1:3
