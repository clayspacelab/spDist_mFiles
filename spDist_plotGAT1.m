...
tpts_to_plot = [0 4.5 12]; % for drawing a white line

delay_epochs = [3.75 5.25; 8.25 9.75; 10.5 12]; % for drawing a bar %delay_tpt_range = [3.75 5.25; 8.25 9.75; 10.5 12];
bar_pos = -2; % what timepoint is the bar aligned at?
...
for aa = 1:length(all_fidelity)
    for vv = 1:length(ROIs)
        ...
        % plot lines for relevant events
        for tt = 1:length(tpts_to_plot)
            % horizontal
            plot(myTR*[tpts(1) (tpts(end)+0.5)],tpts_to_plot(tt)*[1 1],'w-','LineWidth',0.5);
            
            % vertical
            plot(tpts_to_plot(tt)*[1 1],myTR*[tpts(1) (tpts(end)+0.5)],'w-','LineWidth',0.5);
        end
        
        % plot bars for relevant epochs
        for dd = 1:size(delay_epochs,1)
            % plot horizontal bars
            plot(delay_epochs(dd,:),[1 1]*bar_pos,'-','LineWidth',1.5,'Color',bar_color);
            
            % plot vertical bars
            plot([1 1]*bar_pos,delay_epochs(dd,:),'-','LineWidth',1.5,'Color',bar_color);
        end
      
        title(ROIs{vv});
        axis tight square
        %set(gca,'XTick',0:4:24,'YTick',0:4:24,'TickDir','out');
        set(gca,'XTick',tpts_to_plot,'YTick',tpts_to_plot,'TickDir','out');
        if vv == 1
            xlabel('Test time (s)');
            ylabel('Train time (s)');
        else
