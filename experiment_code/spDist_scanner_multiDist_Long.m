...
function spDist_scanner_multiDist_mr(subj,run,task_coh)
...
if p.scanner == 0
    ...
    p.desired_resolution = [1280 1024]; % desired resolution, compare to the 'actual' resolution below [this is for mbp laptop]
...
% 
% p.n_misses_1st = sum(isnan(p.dist_resp_1stDec(p.conditions(:,1)==2)));
% 
% p.acc_1st = nanmean(p.correct(~isnan(p.dist_resp_1stDec)));
% 
...
% p.acc = nanmean([p.acc_1st p.acc_2nd]);
    ...
    p.n_misses = sum(isnan(p.dist_resp_new(p.conditions(:,1)==2)));
