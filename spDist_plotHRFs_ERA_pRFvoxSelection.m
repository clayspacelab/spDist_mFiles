...
function spDist_plotHRFs_ERA_pRFvoxSelection(subj,sess,ROIs)
...
%%% gh add %%%
pol_diff_thresh = 165; %?  or 15, change pol_diff to <=
min_ecc = 7; 
max_ecc = 15; 
%%% gh add %%%
    ...
    for sess_idx = 1:length(sess{ss})
        ...
        for vv = 1:length(ROIs)
            ...
            for pp = 1:length(data.targ_ang_all)
                ...
                tmprel =  data.targ_ang_all(pp) - tmp_pol;
                pol_diff(pp,:)= abs(mod((tmprel+180), 360)-180); %dont care about sign here, just need angular difference
                %updating vox selection criteria using pol ang sep, ecc
                %min/max
                which_vox = data.rf.ve>=ve_thresh & pol_diff(pp,:) >= pol_diff_thresh & tmp_ecc > min_ecc & tmp_ecc < max_ecc;
                
                idx = thisidx(pp);
                all_hrfs(idx,:) = squeeze(nanmean(data.dt_allz(pp,which_vox,:),2))';
...
for vv = 1:length(ROIs)
    ...
    for cc = 1:length(cu)
        ...
        for ss = 1:length(subj)
            ...
            thisd(ss,:) = nanmean(all_hrfs(thisidx,:),1); %using nanmean here 
            all_mean_hrf{cc}(vv,:,ss) = nanmean(all_hrfs(thisidx,:),1);
            clear thisidx;
        end
