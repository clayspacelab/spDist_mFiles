function [F_store] = RMAOV33_gh(X,alpha)
...
F_store =[]; %initialize storage container for Fvals 
a = max(X(:,2));
b = max(X(:,3));
c = max(X(:,4));
s = max(X(:,5));
